        ...
        <div className="space-y-5 print:hidden">
          ...
          {definition.sections.map((section) => (
            <section key={section.title} className="rounded-2xl border bg-white p-5 shadow-sm">
              <div className="flex items-center justify-between gap-3"><h3 className="text-base font-bold text-gray-950">{section.title}</h3>{(["quotation", "payment-statement"].includes(type) && section.title === "수신 기관") && <button type="button" onClick={loadLoginInfo} className="whitespace-nowrap rounded-md border border-teal-200 px-2.5 py-1.5 text-xs font-bold text-teal-800 hover:bg-teal-50">로그인 정보 불러오기</button>}{type === "venue-guide" && section.title === "기본 정보" && <button type="button" onClick={loadLoginInfo} className="whitespace-nowrap rounded-md border border-teal-200 px-2.5 py-1.5 text-xs font-bold text-teal-800 hover:bg-teal-50">로그인 정보 불러오기</button>}{type === "safety-education" && section.title === "교육 개요" && <button type="button" onClick={loadLoginInfo} className="whitespace-nowrap rounded-md border border-teal-200 px-2.5 py-1.5 text-xs font-bold text-teal-800 hover:bg-teal-50">로그인 정보 불러오기</button>}{type === "parent-education" && section.title === "발행 정보" && <button type="button" onClick={loadLoginInfo} className="whitespace-nowrap rounded-md border border-teal-200 px-2.5 py-1.5 text-xs font-bold text-teal-800 hover:bg-teal-50">로그인 정보 불러오기</button>}{type === "quotation" && section.title === "견적 내역" && <button type="button" onClick={loadCart} className="whitespace-nowrap rounded-md border border-damda-yellow px-2.5 py-1.5 text-xs font-bold text-gray-900 hover:bg-amber-50">장바구니 내역 불러오기</button>}{type === "payment-statement" && section.title === "결제 금액" && <button type="button" onClick={loadLatestReservation} className="whitespace-nowrap rounded-md border border-damda-yellow px-2.5 py-1.5 text-xs font-bold text-gray-900 hover:bg-amber-50">예약 내역 불러오기</button>}</div>
