...
export function TeacherPracticalInfo({ product }: { product: Product }) {
  ...
  const rows = ([
    ...
    { icon: CloudRain, label: "우천 시 운영", value: YES_NO(product.operates_in_rain) },
    { icon: CloudRain, label: "우천 대체 프로그램", value: product.rain_alternative },
    { icon: Bus, label: "버스 진입", value: YES_NO(product.bus_accessible) },
    { icon: Bus, label: "버스 주차", value: YES_NO(product.bus_parking_available) },
    { icon: Bus, label: "승하차 공간", value: YES_NO(product.dropoff_space_available) },
    { icon: Sandwich, label: "현장 식사", value: YES_NO(product.meal_available) },
    { icon: Sandwich, label: "도시락 지참", value: YES_NO(product.lunchbox_allowed) },
    { icon: Toilet, label: "유아용 화장실", value: YES_NO(product.child_restroom_available) },
